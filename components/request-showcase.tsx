...
function ShowcaseItem({ requestText, imgSrc, title, description, tags, link }: ShowcaseItemProps) {
  ...
  return (
    <div ref={itemRef} className="max-w-4xl mx-auto relative z-10 mb-20 last:mb-0">
      <div
        className={`mb-8 transition-all duration-700 delay-200 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"
          }`}
      >
      ...
      <div
        className={`flex justify-center my-8 transition-all duration-700 delay-400 ${isVisible ? "opacity-100 scale-100" : "opacity-0 scale-50"
          }`}
      >
        ...
        <div className="group relative rounded-xl overflow-hidden border-2 border-primary/40 hover:border-primary/70 transition-all duration-500 hover:scale-[1.02] shadow-2xl shadow-primary/20 hover:shadow-primary/30">
          ...
          <div className="p-6 bg-card/80 backdrop-blur-sm border-t border-primary/20">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {tags.map((tag, i) => (
                <span key={i} className={`px-3 py-1 text-sm rounded-full border ${i === 1 ? 'bg-cyan-500/30 text-cyan-400 border-cyan-400/40' : 'bg-primary/30 text-primary border-primary/40'
                  }`}>
                  {tag}
                </span>
              ))}
      ...
      <div
        className={`text-center mt-12 transition-all duration-700 delay-800 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
          }`}
      >
        <p className="text-foreground/70 mb-4">단순한 요청만으로도 이보다 더 뛰어난  웹사이트를 제작해드립니다</p>
        <Button
          size="lg"
...
export function RequestShowcase() {
  ...
  return (
    <section id="showcase" className="py-20 px-4 relative overflow-hidden">
      ...
      <div ref={headerRef} className="max-w-4xl mx-auto relative z-10 mb-16">
        <div
      ...
      <ShowcaseItem
        requestText="IT기업의 소개 페이지를 미래지향적이게 만들어 주세요"
        imgSrc="/cloudops-intro.png"
        title="IT 기업 소개 페이지"
      ...
      <ShowcaseItem
        requestText="sony 헤드폰을 소개하는 랜더링 페이지를 만들어주세요"
        imgSrc="/sony_headphone.png"
        title="소니 헤드폰 소개 페이지"
        description="소니의 최신 헤드폰을 소개하는 몰입형 랜딩 페이지. 프리미엄 디자인과 동적 애니메이션 적용."
        tags={["제품 소개", "반응형", "몰입형 이펙트"]}
